...
def owner_register():
    ...
    if request.method == 'POST':
        ...
        if not user and password == password2:
            ...
            restaurant = Restaurant(name=rname, uid = user.uid, type=1)
...
def add_employee():
    ...
    if request.method == 'POST':
        ...
        password = (hashlib.md5(request.form['password'].encode())).hexdigest()
        user = User.query.filter(User.email == email).first()
        rname = Restaurant.query.filter(Restaurant.name == session['account']).first().rid
        if user.password == password:
            user.type = 0
            restaurant = Restaurant(name=rname, uid=user.uid, type=0)
            db.session.add(restaurant)
            db.session.commit()
            return redirect(url_for('home'))
